...
describe.skip( 'Create item', function(){
	it("should create an item without error", function(){
		console.log('config:');
		...
		d.done(function(item){
			...
			writeItemD.done(function(itemsArray){
				...
				assert.isOk(itemsArray[0].get('itemVersion') > 0, 'We expect to have an updated itemVersion since it\'s on the server now');
				assert.isOk(itemsArray[1].get('itemVersion') > 0, 'We expect to have an updated itemVersion since it\'s on the server now');
				assert.isOk(itemsArray[2].get('itemVersion') > 0, 'We expect to have an updated itemVersion since it\'s on the server now');
				
				assert.isOk(itemsArray[0].get('itemVersion') < itemsArray[2].get('itemVersion'), 'Expect itemVersion for parent item to be less than note items');
				assert.isOk(itemsArray[1].get('itemVersion') == itemsArray[2].get('itemVersion'), 'Expect itemVersion for child notes to be assert.equal');
